    ...
    try {
      ...
      workbook.SheetNames.forEach(sheetName => {
        ...
        for (let i = 0; i < Math.min(20, jsonSheet.length); i++) {
            ...
            const hasClient = row.some(c => String(c).trim() === '«·⁄„Ì·' || String(c).toLowerCase().trim() === 'client');
            const hasDispatch = row.some(c => String(c).trim() === '«·—”«·…' || String(c).trim() === '«·—”«·Â' || String(c).toLowerCase().trim() === 'dispatch');
        ...
        const knownKeywords = {
            client: ['«·⁄„Ì·', 'client', 'customer'],
            color: ['«··Ê‰', 'color', 'col'],
            quantity: ['«·ﬂ„Ì…', '«·ﬂ„ÌÂ', 'quantity', 'qty', 'kg'], 
            dispatch: ['«·—”«·…', '«·—”«·Â', 'dispatch', 'id'],
            sentDate: ['«· «—ÌŒ', 'date', 'sent'],
            formationDate: ['«· ‘ﬂÌ·', 'formation'],
            item: ['«·’‰›', 'item', 'fabric', 'design'],
            received: ['«·„” ·„', 'received', 'rec'],
            remaining: ['«·„ »ﬁÌ', 'remaining', 'balance'],
            waste: ['‰”»… «·Â«·ﬂ', 'waste', 'wastage']
        };
